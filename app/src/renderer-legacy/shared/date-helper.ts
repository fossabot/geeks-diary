...
export class DateHelper {
    ...
    diff(source: Date, target: Date, unit: DateUnit = DateUnit.MILLISECONDS): number {
        ...
        let diffAsMilliseconds = targetTime - sourceTime;
        const isMinus = diffAsMilliseconds < 0;

        diffAsMilliseconds = Math.abs(diffAsMilliseconds);

        const diff = Math.floor(diffAsMilliseconds / DateHelper.unitAsMilliseconds[unit]);

        return isMinus ? -diff : diff;
    }

    isSame(source: Date, target: Date, unit: DateUnit = DateUnit.MILLISECONDS): boolean {
